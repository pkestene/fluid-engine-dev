...
# This file is adopted from Embree project (https://github.com/embree/embree)
...
IF (JET_ISPC_SUPPORT)
    ...
    IF (NOT JET_ISPC_EXECUTABLE)
        ...
        FIND_PROGRAM(JET_ISPC_EXECUTABLE ispc PATHS ${ISPC_DIR_HINT} DOC "Path to the ISPC executable.")
        IF (NOT JET_ISPC_EXECUTABLE)
            MESSAGE(FATAL_ERROR "Intel SPMD Compiler (ISPC) not found. Disable JET_ISPC_SUPPORT or install ISPC.")
        ELSE()
            MESSAGE(STATUS "Found Intel SPMD Compiler (ISPC): ${JET_ISPC_EXECUTABLE}")
        ENDIF()
    ENDIF()

    # check ISPC version
    EXECUTE_PROCESS(COMMAND ${JET_ISPC_EXECUTABLE} --version OUTPUT_VARIABLE ISPC_OUTPUT)
    ...
    GET_FILENAME_COMPONENT(ISPC_DIR ${JET_ISPC_EXECUTABLE} PATH)

    SET(JET_ISPC_ADDRESSING 32 CACHE INT "32vs64 bit addressing in ispc")
    MARK_AS_ADVANCED(JET_ISPC_ADDRESSING)
    ...
    MACRO (ISPC_COMPILE)
        ...
        FOREACH(src ${ARGN})
            ...
            ADD_CUSTOM_COMMAND(
                    ...
                    COMMAND ${JET_ISPC_EXECUTABLE}
                    -I ${CMAKE_CURRENT_SOURCE_DIR}
                    ${ISPC_INCLUDE_DIR_PARMS}
                    ${ISPC_DEFINITIONS}
                    --arch=${ISPC_ARCHITECTURE}
                    --addressing=${JET_ISPC_ADDRESSING}
                    ${ISPC_OPT_FLAGS}
    ...
    MACRO (ADD_JET_ISPC_EXECUTABLE name)
        SET(ISPC_SOURCES "")
        SET(OTHER_SOURCES "")
        FOREACH(src ${ARGN})
